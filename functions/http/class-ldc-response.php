...
if(!class_exists('LDC_Response')){
    class LDC_Response {
        ...
        protected function from_array($response = []){
            ...
            if(!$this->code or !$this->message or $this->success != ldc_seems_successful($this->code)){
                $this->code = 500;
        ...
        public function __construct($response = null){
            ...
            $success = false;
            switch(true){
                case ldc_seems_response($response):
                    $code = $response['code'];
                    $data = $response['data'];
                ...
                case is_a($response, 'Requests_Response'):
                    ...
                    $success = ldc_seems_successful($code);
                    break;
                case is_wp_error($response):
                    $data = $response->get_error_data();
                    $message = $response->get_error_message();
                    break;
                case ldc_seems_wp_http_requests_response($response):
